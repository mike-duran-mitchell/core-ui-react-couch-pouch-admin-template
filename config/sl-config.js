...
module.exports = SLConfig = {
  ...
  security: {
    ...
    lockoutTime: 600,
    // The amount of time a new session is valid for (default: 30 days)
    sessionLife: 43200,
    // The amount of time a password reset token is valid for (default: 12 hours)
    tokenLife: 43200,
    ...
    // If set to true, the user will be logged in automatically aft   er resetting the password
  ...
  local: {
    ...
    confirmEmailRedirectURL: "/dashboard",
  ...
  userModel: {
    ...
    blacklist: ["role", "*design*"],
    sanitize: {
      company: ["trim", "escape", "stripLow"],
      username: ["trim", "toLowerCase", "escape", "stripLow"],
      firstName: ["trim", "escape", "stripLow"],
      lastName: ["trim", "escape", "stripLow"],
      password: ["trim"],
      confirmPassword: ["trim"],
      email: ["escape", "stripLow"]
    },
    validate: {
      username: {
        presence: true,
        length: {
          maximum: 15,
          tooLong: "must be fewer than 15 characters",
          minimum: 6,
          tooShort: "must be at least 6 characters"
        },
        format: {
          pattern: "^[\\w]+$",
          message: "can only contain alphanumeric characters (a-z, 0-9)"
        }
      },
      email: {
        presence: true,
